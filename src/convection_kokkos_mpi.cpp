...
namespace mpi_variables {
    int mpi_size;
    int mpi_rank;
    MPI_Comm comm_cart;
    int mpi_dims[3] = {0,1,1};
    int mpi_periods[3] = {1,1,1};
    int mpi_cart_coord[3];
    int mpi_neighbors[4];
}
...
void fill_data(Mesh const& mesh, Setup const& setup, std::vector<std::vector<double>>& data, ConstArrayHost const& U) {
    ...
    double rhoc, uc, vc, wc, ekinc, egc, Tc; // Density, velocity components, kinetic energy, gravitational energy, temperature
...
void write_output(Mesh const& mesh, Setup const& setup, int it, int freq_output, ConstArray const& U) {
    ...
    if(mpi_rank==0) { 
        MPI_Reduce(MPI_IN_PLACE, &ekin_max, 1, MPI_DOUBLE, MPI_MAX, 0, comm_cart);
        std::cout << "timestep: " << it << " kinetic energy: " << ekin_max << '\n';
    } else {
        MPI_Reduce(&ekin_max, &ekin_max, 1, MPI_DOUBLE, MPI_MAX, 0, comm_cart);
    }
    ...
    if (it % freq_output == 0) {
        ...
        std::stringstream ss_mpi_rank;
        ss_mpi_rank << std::setfill('0') << std::setw(3) << mpi_rank;
        std::stringstream ss_iout;
        ss_iout << std::setfill('0') << std::setw(6) << iout;
        output_file = "output.csv." + ss_mpi_rank.str() + "." + ss_iout.str();
...
void compute_boundary_condition(Mesh const& mesh, Setup const& setup, Array const& U, Kokkos::View<double***> sub_view_d, Kokkos::View<double***, Kokkos::HostSpace> sub_view_h) {
    using namespace conv_variables;
    using namespace mpi_variables;
    ...
    auto sub_view = Kokkos::subview(U, 1, Kokkos::ALL, Kokkos::ALL, Kokkos::ALL);
    auto sub_view_2 = Kokkos::subview(U, mesh.nx+1, Kokkos::ALL, Kokkos::ALL, Kokkos::ALL);
    Kokkos::deep_copy(sub_view_d, sub_view);
    Kokkos::deep_copy(sub_view_h, sub_view_d);

    MPI_Sendrecv_replace(sub_view_h.data(), (mesh.ny+2)*(mesh.nz+2)*nvar, MPI_DOUBLE, mpi_neighbors[0], 0, 
                         mpi_neighbors[1], 0, comm_cart, MPI_STATUS_IGNORE);
    Kokkos::deep_copy(sub_view_d, sub_view_h);
    Kokkos::deep_copy(sub_view_2, sub_view_d);


    auto sub_view_3 = Kokkos::subview(U, mesh.nx, Kokkos::ALL, Kokkos::ALL, Kokkos::ALL);
    auto sub_view_4 = Kokkos::subview(U, 0, Kokkos::ALL, Kokkos::ALL, Kokkos::ALL);
    Kokkos::deep_copy(sub_view_d, sub_view_3);
    Kokkos::deep_copy(sub_view_h, sub_view_d);

    MPI_Sendrecv_replace(sub_view_h.data(), (mesh.ny+2)*(mesh.nz+2)*nvar, MPI_DOUBLE, mpi_neighbors[1], 0, 
                         mpi_neighbors[0], 0, comm_cart, MPI_STATUS_IGNORE);
    Kokkos::deep_copy(sub_view_d, sub_view_h);
    Kokkos::deep_copy(sub_view_4, sub_view_d);
...
double compute_timestep(Mesh const& mesh, Setup const& setup, ConstArray const& U) {
    ...
    MPI_Allreduce(MPI_IN_PLACE, &dt, 1, MPI_DOUBLE, MPI_MIN, comm_cart); // Reduce the local time step to get the global time step
    return dt;
}
...
int main(int argc, char * argv[]) {
    ...
    mesh.nx = 100;  // Number of grid points in x-direction
    mesh.ny = 1;    // Number of grid points in y-direction
    mesh.nz = 50;   // Number of grid points in z-direction
    nt = 5000; // Total number of time steps to simulate
    ...
    MPI_Cart_create(MPI_COMM_WORLD, 3, mpi_dims, mpi_periods, 0, &comm_cart);
    MPI_Cart_coords(comm_cart, mpi_rank, 3, mpi_cart_coord);
    MPI_Cart_shift(comm_cart, 0, 1, &mpi_neighbors[0], &mpi_neighbors[1]);
    // MPI_Cart_shift(comm_cart, 1, 1, &mpi_neighbors[2], &mpi_neighbors[3]);

    if(mesh.nx%mpi_dims[0]+mesh.ny%mpi_dims[1]!=0 && mpi_rank==0)
    {
        std::cout<<"configuration not compatible: nx%mpi_dims[0]+ny%mpi_dims[1]+nz%mpi_dims[2]!=0\n";
        return 0;
    }

    mesh.nx = mesh.nx/mpi_dims[0];
    mesh.ny = mesh.ny/mpi_dims[1];
    mesh.nz = mesh.nz/mpi_dims[2];
    ...
    Kokkos::View<double***> sub_view_d("buffer_X", mesh.ny + 2, mesh.nz+2, nvar);
    Kokkos::View<double***, Kokkos::HostSpace> sub_view_h("buffer_X_h", mesh.ny + 2, mesh.nz+2, nvar);
    
    linspace(-0.5 * mesh.dx + mpi_cart_coord[0] * mesh.nx * mesh.dx, (mpi_cart_coord[0]+1) * mesh.nx * mesh.dx + 0.5 * mesh.dx, mesh.nx + 2, mesh.xc); // Generate x-coordinates
    linspace(-0.5 * mesh.dy + mpi_cart_coord[1] * mesh.ny * mesh.dy, (mpi_cart_coord[1]+1) * mesh.ny * mesh.dy + 0.5 * mesh.dy, mesh.ny + 2, mesh.yc); // Generate y-coordinates
    linspace(-0.5 * mesh.dz + mpi_cart_coord[2] * mesh.nz * mesh.dz, (mpi_cart_coord[2]+1) * mesh.nz * mesh.dz + 0.5 * mesh.dz, mesh.nz + 2, mesh.zc); // Generate z-coordinates
    ...
    compute_boundary_condition(mesh, setup, Uold, sub_view_d, sub_view_h);
